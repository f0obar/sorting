...
public class Sorter extends Task {
    ...
    private void heapSort() {
        heapSortGenerateHeap();
        for(int i = values.length -1; i > 0; i--) {
            swap(i,0);
            heapSortShiftDown(0, i);
        }
    }

    private void heapSortGenerateHeap() {
        for(int i = (values.length / 2) - 1; i >= 0 ; i--) {
            heapSortShiftDown(i, values.length);
        }
    }

    private void heapSortShiftDown(int i, int n) {
        while (i <= (n / 2) - 1) {
            int child = ((i + 1) * 2) - 1;
    ...
    private void set(int index, int value){
        long delay = System.nanoTime();
        //active waiting because of windows scheduler reasons.
        while (System.nanoTime() < (delay + controller.delaySlider.getValue()*5000)){}

        accesses+= 1;
        Platform.runLater(() -> controller.updateAccesses(accesses));

        values[index] = value;
        Platform.runLater(() -> controller.updateLine(index, values[index]));
    }
